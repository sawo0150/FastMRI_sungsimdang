...
def train_epoch(args, epoch, model, data_loader, optimizer, loss_type):
    ...
    for iter, data in enumerate(data_loader):
        ...
        kspace = kspace.to('cpu', non_blocking=True)
        target = target.to('cpu', non_blocking=True)
        maximum = maximum.to('cpu', non_blocking=True)
...
def train(args):
    ...
    print('Current cuda device: ', torch.cpu.current_device())
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        val_loss = torch.tensor(val_loss).to('cpu')
        num_subjects = torch.tensor(num_subjects).to('cpu')
