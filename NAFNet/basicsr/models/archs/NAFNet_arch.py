...
class NAFNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[]):
        super().__init__()
        # self.function = nn.Conv2d(in_channels=img_channels, out_channels=1, kernel_size=3, padding=1, stride=1, bias=True)
        #torch.nn.init.kaiming_normal_(self.function.weight, mode='fan_in', nonlinearity='relu')
        #torch.nn.init.constant_(self.function.bias, 0)
        self.relu = nn.ReLU()
    ...
    def forward(self, inp):
        ...
        x = x + inp[:,0,:H,:W] # 최종 출력 = 기존 + residual
